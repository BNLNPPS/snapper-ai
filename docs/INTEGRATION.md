...
| `curve_groups` | curve family rows of the observatory legend; a callable is resolved per render so families track live host state. A family entry may declare `order` (member display order), `window_relative` (cumulative counters rendered relative to the window's left edge — `True` for the whole family, or a list of ids and `_`-terminated prefixes within a mixed family), `default_off` (the whole family starts unticked), `default_off_ids` (individual members stay unticked until explicitly enabled), `tall` (double panel height), `focus_closed` (section closed by default inside a focus view), `event_flow_bin_scale` (an event-flow family's display bins are this many times the page's ladder rung, so a sparse family draws as bars rather than hairlines; the page takes the largest declared), `event_flow` (the family plots discrete events from the provider's `event_values` hook, binned by the series at the native rung; an event is a stamp, a [stamp, qualifier] pair, or a [stamp, qualifier, weight] triple, and a bin carries the count with its per-qualifier breakdown and, when any event carried a weight, the summed weight with its breakdown), `measures` with `units_by_measure` and `title_by_measure` (the bin measures an event-flow family plots, `count` and `weight`, one panel per measure down the page from the same curves, with the family's unit and title under each; `measure_param` names instead a focus selector axis that picks a single measure), `cumulative_panel` (an event-flow family draws a second panel beneath its own: its bins integrated from the left edge of the view, stacked by the same members in the same measure and under the same chips, re-based when the view moves; the panel has its own docked title row and no curves of its own) with `aggregate_detail_key` (the cut card element that docks beneath the integrated panel, as `detail_key` names the one beneath the family's panel; the cut request carries the integration range as `agg_from` and `agg_to`), `selector_group` (the key by which a family-scoped selector axis finds this family, for a panel kind that repeats per entity — the axis names the group rather than one family name), and `fills` (a map of member id → hatch: `pattern`, `fgcolor`, `bgcolor`, `size`, `solidity` — a hatched area from zero to that member's curve, marking a member that is a subset of a sibling; the hatch distinguishes it from a stacked band, and filled members draw beneath their siblings' lines) |
...
| `focus_view` | one declaration or a tuple: scope-switcher tabs, each with its own clean page, options, selector axes, and an optional one-line `note` explaining the display. `label` names the tab and the URL slug; optional `view_label` names the page when it is longer than the tab (the heading and title read `<view_label> view`); optional `selector_label` names the selectable entities when those concepts differ. The focus parameter accepts `all` as a durable selection of every currently registered option and `none` as every option off; the page writes those two words into the URL in place of the full list or an empty value, and the cut expands them for the card builders. A declaration may name `all` as its `default`. A selector axis declaring `families: False` chooses nothing about which families render (a measure switch, say) and stays out of the families key by which options name their families. An axis declaring `family` is not a page control: it renders in that family's own docked control row, beside its ticks and its rotator, and governs that panel alone — the form for a grouping that is meaningful for one panel and degenerate for another. It names the family by registry name or by the `selector_group` several families share, and rides the URL, the cut fetch, window stepping and remembered preferences exactly as a page-level axis does. An option's `families_by` is a map keyed by the families key, or a callable over the selected values `{param: value}` — the form a view with several axes takes, resolving its families directly instead of precomputing every combination of them. A declaration may name a `default_window` (a named window key) for its clean page, the view's natural span; it outranks the remembered scope window, and an option's `start` floor still applies. Section and jump-list peaks render compactly, millions as 1.3M. The clean page restores the signed-in user's last selection for the focus ahead of the default; the client does the same from local storage for a visitor not signed in. An option may declare an `activity` curve id (and the declaration an `activity_label`): with several options selected, presentation follows activity — options ordered by the peak of that curve over the shown window, idle options last in alphabetical order with their sections closed — and a jump list under the selector, in the same order with each peak in brackets, scrolls to an option's first section. An option may declare `pin: 'last'`: it stays out of the activity ranking, its section renders after the ranked options and is never marked idle, and its jump entry carries no peak — the form for scope-level families riding a per-entity view. An optional `member_restrict` hook — called with the request query, returning `None` or `{'note', 'keep', 'params', 'group_ids', 'group_titles'}` — narrows the plotted members per request (the cached series stays whole): curves failing the `keep` predicate drop, the `note` renders as the page's visible statement of the restriction, the `params` ride the cut fetch and window stepping so the restriction survives navigation, and `group_ids`/`group_titles` remap a family's exact-id membership and title for the request (an aggregate-based family substituting per-member curves under restriction — such a family declares the substitutes' prefixes as `extra_cache_prefixes`, captured in the cached focus series without joining the family's display). A family may also declare `empty_note`, rendered centered in its panel when its members hold points but no nonzero value in the window. |
